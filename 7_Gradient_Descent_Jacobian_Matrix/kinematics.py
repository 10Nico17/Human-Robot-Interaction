...
def get_jacobian(q:np.ndarray, mode:int=MODE_2D, ee_module_length=0) -> np.ndarray:
    ...
    # catch illegal mode
    if mode != MODE_2D:
        print('ERROR: Cannot compute Jacobian outside 2D mode!')
        return None
    ...
    LINK_LENGTH_1       = 72.5
    LINK_LENGTH_2       = 72.5
    LINK_LENGTH_3       = 21 + ee_module_length
        
    dx_dq0 = 0
    dx_dq1 = LINK_LENGTH_1 * -np.sin(np.pi/2. + q[1]) + LINK_LENGTH_2 * -np.sin(np.pi/2. + q[1] + q[2]) + LINK_LENGTH_3 * -np.sin(np.pi/2. + q[1] + q[2] + q[3])
    dx_dq2 =                                            LINK_LENGTH_2 * -np.sin(np.pi/2. + q[1] + q[2]) + LINK_LENGTH_3 * -np.sin(np.pi/2. + q[1] + q[2] + q[3])
    dx_dq3 =                                                                                              LINK_LENGTH_3 * -np.sin(np.pi/2. + q[1] + q[2] + q[3])

    dy_dq0 = 0
    dy_dq1 = LINK_LENGTH_1 *  np.cos(np.pi/2. + q[1]) + LINK_LENGTH_2 *  np.cos(np.pi/2. + q[1] + q[2]) + LINK_LENGTH_3 *  np.cos(np.pi/2. + q[1] + q[2] + q[3])
    dy_dq2 =                                            LINK_LENGTH_2 *  np.cos(np.pi/2. + q[1] + q[2]) + LINK_LENGTH_3 *  np.cos(np.pi/2. + q[1] + q[2] + q[3])
    dy_dq3 =                                                                                              LINK_LENGTH_3 *  np.cos(np.pi/2. + q[1] + q[2] + q[3])
